...
PS_INPUT VS(VS_INPUT input)
{
    ...
    output.Normal = eye_pos; //mul(input.Normal, world);
...
float4 PS(PS_INPUT input) : SV_Target
{
	...
	float diffuse_monochrome = pow(saturate(dot(normal, light)), 10);
    float3 diffuse_value = saturate(dot(normal, light));
	float specular_monochrome = pow(saturate(dot(normal, halfway)), 60);
    float3 specular = pow(saturate(dot(normal, halfway)), 60);
    	
    float4 diffuse = diffuse_color;
	return float4(lerp(
		float3(0.1, 0.1, 0.1),
		float3(0.2, 0.3, 0.2), diffuse_monochrome), diffuse.a);
    return float4(0.7*specular + diffuse_value * diffuse, diffuse.a);
}
