...
DebugRenderer::DebugRenderer(ID3D10DevicePtr device)
  : device_(device)
  , effect_(new EffectWrapper(device))
  , vector_font_(createVectorFont())
{

  D3DX10CreateFont( device_, 15, 0, FW_BOLD, 1, FALSE, DEFAULT_CHARSET, 
...
void  DebugRenderer::add_line(const D3DXVECTOR3& a, const D3DXVECTOR3& b, const D3DXCOLOR& color, const D3DXMATRIX& view_proj)
{
  ...
  add_verts(Pos | Color, D3D10_PRIMITIVE_TOPOLOGY_LINELIST, &pts[0].pos.x, 2, kMtxId, view_proj);
}
                                         ...
                                         const D3DXMATRIX& world, const D3DXMATRIX& view_proj)
{
  ...
  color_sphere(color);
  add_verts(Pos | Color, D3D10_PRIMITIVE_TOPOLOGY_LINELIST, sphere_verts_[0].pos, sphere_verts_.size(), world2 * world, view_proj);
}

void DebugRenderer::add_wireframe_sphere(const D3DXMATRIX& world, const D3DXCOLOR& color, const D3DXMATRIX& view_proj)
{
  color_sphere(color);
  add_verts(Pos | Color, D3D10_PRIMITIVE_TOPOLOGY_LINELIST, sphere_verts_[0].pos, sphere_verts_.size(), world, view_proj);
}

ID3D10Buffer* DebugRenderer::create_dynamic_vertex_buffer(const uint32_t vertex_count, const uint32_t vertex_size)
{
  ...
  if (FAILED(device_->CreateBuffer(&buffer_desc, NULL, &vertex_buffer))) {
    return NULL;
  }
...
bool DebugRenderer::init()
{
  ...
  blend_state_.Attach(rt::D3D10::BlendDescription()
    .BlendEnable_(0, FALSE)
    .Create(device_));

  depth_stencil_state_.Attach(rt::D3D10::DepthStencilDescription()
    .Create(device_));
...
bool DebugRenderer::init_vertex_buffers()
{
  ...
  D3D10_PASS_DESC pass_desc;
  {
    ...
    data.vertex_buffer_ = create_dynamic_vertex_buffer(data.num_verts, data.vertex_size_);
    ...
    if (!create_input_layout<mpl::vector<D3DXVECTOR3> >(data.input_layout_, pass_desc, device_)) {
      return false;
    }
  }
  ...
  // pos color
  {
    ...
    if (!create_input_layout<mpl::vector<D3DXVECTOR3, D3DXCOLOR> >(data.input_layout_, pass_desc, device_)) {
      return false;
    }
  }
...
void DebugRenderer::start_frame()
{
  ...
  for (VertexFormats::iterator i = vertex_formats_.begin(), e = vertex_formats_.end(); i != e; ++i) {
    ...
    if (FAILED(cur.vertex_buffer_->Map(D3D10_MAP_WRITE_DISCARD, 0, (void**)&cur.data_))) {
...
void DebugRenderer::end_frame()
{
  for (VertexFormats::iterator i = vertex_formats_.begin(), e = vertex_formats_.end(); i != e; ++i) {
    VertexFormatData& cur = i->second;
    cur.vertex_buffer_->Unmap();
  }
}

void DebugRenderer::add_verts(const uint32_t vertex_format, const D3D10_PRIMITIVE_TOPOLOGY topology, 
                              const float* verts, const uint32_t vertex_count, const D3DXMATRIX& world, const D3DXMATRIX& view_proj)
{
  ...
  const bool list_format = topology == D3D10_PRIMITIVE_TOPOLOGY_LINELIST || topology == D3D10_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
  bool stitched = false;
...
void DebugRenderer::render()
{
  ...
  device_->OMSetDepthStencilState(depth_stencil_state_, 0xffffffff);
  ...
  for (VertexFormats::iterator i = vertex_formats_.begin(), e = vertex_formats_.end(); i != e; ++i) {
    ...
    if (cur.vertex_count_ > 0) {
      ...
      device_->IASetInputLayout(cur.input_layout_);
      ID3D10Buffer* bufs[] = { cur.vertex_buffer_ };
      ...
      for (It i_top = cur.draw_calls_by_topology_.begin(), e_top = cur.draw_calls_by_topology_.end(); i_top != e_top; ++i_top) {
        ...
        device_->IASetPrimitiveTopology(i_top->first);
        ...
        for (CallsIt i_call = i_top->second.begin(), e_call = i_top->second.end(); i_call != e_call; ++i_call) {
          ...
          device_->Draw(i_call->vertex_count, i_call->start_vertex_location);
        }
      }
    }
  }
...
void DebugRenderer::add_text(const D3DXVECTOR3& pos, const D3DXMATRIX& view, const D3DXMATRIX& proj, const bool billboard, const char* format, ...)
{
  ...
  if (billboard) {
    ...
    add_verts(Pos, D3D10_PRIMITIVE_TOPOLOGY_LINELIST, &verts[0], verts.size() / 3, kMtxId, rot * trans * view * proj);
  } else {
    add_verts(Pos, D3D10_PRIMITIVE_TOPOLOGY_LINELIST, &verts[0], verts.size() / 3, kMtxId, trans * view * proj);
  }
}
